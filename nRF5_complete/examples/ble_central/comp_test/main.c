...
static uint8_t *decomp_buffers[NUM_DATA_BUFFERS];
...
void buffers_init(void)
{
	for(int i = 0; i < NUM_DATA_BUFFERS; i++)
	{
		...
		/*decomp_buffers[i] = (uint8_t *)malloc(TRANSMISSION_BUF_SIZE);
		if(decomp_buffers[i] == NULL)
		{
			printf("Decomp buffer %d Alloc failed\r\n", i);
		}
		memset(decomp_buffers[i], 0, TRANSMISSION_BUF_SIZE);*/
...
void compress(uint32_t buf, uint32_t size)
{
    ...
    uint32_t polled = 0;
	  ...
	  /*for(int i = 0; i < size; i++)
		{
			printf("%d ", data_buffers[buf][i]);
			nrf_delay_ms(1);
			//while(app_uart_put(data_buffers[buf][i]) != NRF_SUCCESS);
		}*/
	  //memset(transmission_buffers[buf], 0, TRANSMISSION_BUF_SIZE);
		//memset(decomp_buffers[buf], 0, TRANSMISSION_BUF_SIZE);
...
/*		uint32_t compressed_size = polled;
    ...
    while (sunk < compressed_size) {
        ASSERT(heatshrink_decoder_sink(&hsd, &(transmission_buffers[buf][sunk]), compressed_size - sunk, &count) >= 0);
        sunk += count;
        ...
        HSD_poll_res pres;
        do {
            pres = heatshrink_decoder_poll(&hsd, &(decomp_buffers[buf][polled]),
                TRANSMISSION_BUF_SIZE - polled, &count);
            ASSERT(pres >= 0);
            polled += count;
    ...
    for (size_t i=0; i<size; i++) {
        if (data_buffers[buf][i] != decomp_buffers[buf][i]) {
            printf("*** mismatch at %zd \r\n", i);
						//show_error();
        }
        ASSERT_EQ(data_buffers[buf][i], decomp_buffers[buf][i]);
    }
		//printf("done\r\n");*/
...
/*void decompress(uint32_t buf, uint32_t size)
{
...
static void data_send(uint8_t buf, uint32_t size)
{
	...
	while(i < size || (i % 20) != 0)
	{
		if(i < size)
		{
			while(app_uart_put(transmission_buffers[buf][i]) != NRF_SUCCESS);
			//printf("%c", transmission_buffers[buf][i]);
		}
		else
		{
	...
	for(int i = 0; i < 20; i++)
	{
		while(app_uart_put(buf_delimiter[i]) != NRF_SUCCESS);
	}
	
	tx_buffers_pending[buf] = EMPTY_TX_BUFF_VAL;
