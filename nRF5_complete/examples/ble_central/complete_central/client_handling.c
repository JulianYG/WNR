...
static void on_evt_hvx(ble_evt_t * p_ble_evt, client_t * p_client, uint32_t index)
{
    ...
    if ((p_client != NULL) && (p_client->state == STATE_RUNNING))
    {
			  ...
			  for(int i = 0; i < 20; i++)
				{
					while(app_uart_put(p_ble_evt->evt.gattc_evt.params.hvx.data[i]) != NRF_SUCCESS);
				}
				//printf("\r\n%x, %x\r\n", p_ble_evt->evt.gattc_evt.params.hvx.data[18], p_ble_evt->evt.gattc_evt.params.hvx.data[19]);
				//while(app_uart_put(p_ble_evt->evt.gattc_evt.params.hvx.data[17]) != NRF_SUCCESS);
			  //printf("%d", p_client->handle.connection_id);
			  //printf("%d\r\n", p_ble_evt->evt.gattc_evt.params.hvx.len);
			  //printf("%d bytes received from conn %d\r\n", p_ble_evt->evt.gattc_evt.params.hvx.len, p_client->handle.connection_id);
        if (
                (
