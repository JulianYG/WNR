...
#define CENTRAL_LINK_COUNT               DEVICE_MANAGER_MAX_CONNECTIONS                 /**<number of central links used by the application. When changing this number remember to adjust the RAM settings*/
...
/***** Handling Defs */
#define NUM_DATA_BUFFERS 2                   // Number of buffers for each device; we assume 2 devices currently.
#define DATA_BUF_SIZE 	(2048U)                                                /**<buffer size for each data buffer.> */
#define TRANSMISSION_BUF_SIZE 	(DATA_BUF_SIZE + (DATA_BUF_SIZE/2) + 4) 

static uint8_t *data_buffers_0[NUM_DATA_BUFFERS];
static uint8_t *data_buffers_1[NUM_DATA_BUFFERS];
uint8_t *pdata_buffers_0;
uint8_t *pdata_buffers_1;
...
//#endif // ENABLE_DEBUG_LOG_SUPPORT
            ...
            //printf("[APPL]:[CI 0x%02X]: Requesting GATT client create\r\n",
            //         p_handle->connection_id);
            ...
            //printf("[APPL]:[0x%02X] << DM_EVT_CONNECTION\r\n", p_handle->connection_id);
            break;
        case DM_EVT_DISCONNECTION:
            //printf("[APPL]:[0x%02X] >> DM_EVT_DISCONNECTION\r\n", p_handle->connection_id);
            ...
            //printf("[APPL]:[0x%02X] << DM_EVT_DISCONNECTION\r\n", p_handle->connection_id);
						...
						//printf("Packets received: %d", counter);
            break;
        ...
        case DM_EVT_SECURITY_SETUP_COMPLETE:
            //printf("[APPL]:[0x%02X] >> DM_EVT_SECURITY_SETUP_COMPLETE, result 0x%08X\r\n",
             //         p_handle->connection_id, event_result);
            //printf("[APPL]:[0x%02X] << DM_EVT_SECURITY_SETUP_COMPLETE\r\n",
            //          p_handle->connection_id);
            break;
        case DM_EVT_LINK_SECURED:
            //printf("[APPL]:[0x%02X] >> DM_LINK_SECURED_IND, result 0x%08X\r\n",
            //          p_handle->connection_id, event_result);
            //printf("[APPL]:[0x%02X] << DM_LINK_SECURED_IND\r\n", p_handle->connection_id);
            break;
        case DM_EVT_DEVICE_CONTEXT_LOADED:
            //printf("[APPL]:[0x%02X] >> DM_EVT_LINK_SECURED\r\n", p_handle->connection_id);
            APP_ERROR_CHECK(event_result);
            //printf("[APPL]:[0x%02X] << DM_EVT_DEVICE_CONTEXT_LOADED\r\n", p_handle->connection_id);
            break;
        case DM_EVT_DEVICE_CONTEXT_STORED:
            //printf("[APPL]:[0x%02X] >> DM_EVT_DEVICE_CONTEXT_STORED\r\n", p_handle->connection_id);
            APP_ERROR_CHECK(event_result);
            //printf("[APPL]:[0x%02X] << DM_EVT_DEVICE_CONTEXT_STORED\r\n", p_handle->connection_id);
            break;
        case DM_EVT_DEVICE_CONTEXT_DELETED:
            //printf("[APPL]:[0x%02X] >> DM_EVT_DEVICE_CONTEXT_DELETED\r\n", p_handle->connection_id);
            APP_ERROR_CHECK(event_result);
            //printf("[APPL]:[0x%02X] << DM_EVT_DEVICE_CONTEXT_DELETED\r\n", p_handle->connection_id);
            break;
        default:
    ...
    switch (p_ble_evt->header.evt_id)
    {
        case BLE_GAP_EVT_ADV_REPORT:
        {
            ...
            data_t type_data;
					  ...
					  //printf("ble BLE_GAP_EVT_ADV_REPORT\r\n");
        ...
        case BLE_GAP_EVT_TIMEOUT:
					  //printf("ble BLE_GAP_EVT_TIMEOUT\r\n");
...
static void buffers_init(void)
{
	...
	{
		data_buffers_0[i] = (uint8_t *)malloc(TRANSMISSION_BUF_SIZE);
		if(data_buffers_0[i] == NULL)
		{
			printf("Data buffer 0 num %d Alloc failed\r\n", i);
		}
		memset(data_buffers_0[i], 0 , TRANSMISSION_BUF_SIZE);
		
		data_buffers_1[i] = (uint8_t *)malloc(TRANSMISSION_BUF_SIZE);
		if(data_buffers_1[i] == NULL)
		{
			printf("Data buffer 1 num %d Alloc failed\r\n", i);
		}
		memset(data_buffers_1[i], 0 , TRANSMISSION_BUF_SIZE);
	}
}
